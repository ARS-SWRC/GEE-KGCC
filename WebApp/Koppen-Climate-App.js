...
function createInfoPanelContent(){
  ...
  return [
    ...
    makeHeader('Usage'),
    makeParagraph('• CLICK TOOLS: Checking these boxes will change the behavior of the cursor so that it retrieves information at a location the user clicks on. Unchecking the boxes will return the cursor to the default behavior where the user can pan the map by dragging it.'),
    ...
    makeParagraph('• COMPARE: Checking this box brings up selection options for Scenario B, allowing Scenario A and B to be compared in the same map window using the split panel slider.'),
    ...
    makeParagraph('• MAP: Note the features in the top-right that allow layers to be toggeled on/off, setting layer transparency, and changing the basemap.'),
